---
title: "Lab 07: Multinomial logistic regression"
subtitle: "due Monday, November 15 at 11:59pm"
output: 
  tufte::tufte_html:
    css: "./sta210-labs.css"
...
- Use multiple logistic regression to explore the relationship between a binary response variable and multiple predictor variables
- Conduct exploratory data analysis for multinomial logistic regression 
- Interpret coefficients of the multinomial logistic regression model
...
2.  Let's prepare the data for analysis and modeling.
    ...
    -   We want to use mulitnomial logistic regression to understand the factors related to how consistently a person votes . The variable `voter_category` identifies the respondent's past voter behavior. Relevel the variable, so "rarely/never" is the baseline level. 
...
3.  Fit a model using mean-centered age, race, gender, income, and education to predict voter category. Show the code used to fit the model, but do **not** display the model output. 

4.  *Should party identification be added to the model?* Use a drop-in-deviance test to determine if party identification should be added to the model. Include the hypotheses in mathematical notation, the output from the test, and the conclusion in the context of the data. Then, display the model you selected. 
...
1. Let's use the model to predict the voting categories. Obtain the predicted voter category for each observation. 
